                        ...
                        <table id="table_received_header" class="table received_header_table">
                            <tr>
                                <th class="th_border">
                                    ...
                                    <div class="row">
                                        <label for="supplier_code" class="control-label col-sm-3">Nhân viên <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                                        <div class="col-sm-9">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-9">
                                            <select id="stock" class="form-control"></select>
                                        </div>
                                    </div>
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-2">
                                            <input type="text" name="total_weight" id="total_weight" class="form-control money" style="background-color: rgba(251,240,83,0.52)" 
                                                onchange="weight_change();" onkeyup="addNewItem(event)" required>
                                            <!-- <div class="input-group">
...
@push('bottom')
	...
	<script type="application/javascript">
		...
		function resume(id) {
            $.ajax({
                ...
                success: function (data) {
                    if (data){
						if(data.received){
                            ...
                            $('#stock').val(data.received.stock_id);
                            $('#user').val(data.received.supplier_id);
                ...
                error: function (request, status, error) {
                    swal("Thông báo","Có lỗi xãy ra khi phục hồi phiếu nhập, vui lòng thử lại.","warning");
                }
            });
        }
        ...
        function showModalproduct_id() {
            var url_product_id = "{{action('AdminGoldStocksReceivedOtherController@getModalData')}}/gold_stocks_received/modal-data?table=gold_products&columns=id,product_code,product_name&name_column=product_id&where=deleted_at+is+null&select_to=product_code:product_code&columns_name_alias=Mã sản phẩm,Tên sản phẩm";
        ...
        function getReceivedHeader() {
            return {
                ...
                supplier_id: $('#user').val() ? Number($('#user').val()) : null,
                stock_id: $('#stock').val() ? Number($('#stock').val()) : null,
        ...
        function printReceived() {
            if(received_id) {
                popupWindow("{{action('AdminGoldStocksReceivedOtherController@getPrintReceived')}}/" + received_id,"print");
            }else{
                alert("Bạn không thể in hóa đơn nếu chưa lưu phiếu nhập!");
            }
        }

        function printReportDetail() {
            if(received_id) {
                popupWindow("{{action('AdminGoldStocksReceivedOtherController@getPrintReceivedDetail')}}/" + received_id,"print");
            }else{
                alert("Bạn không thể in bảng kê chi tiết nếu chưa lưu phiếu nhập!");
            }
        }
