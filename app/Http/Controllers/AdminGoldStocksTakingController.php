<?php namespace App\Http\Controllers;
	...
	class AdminGoldStocksTakingController extends CBExtendController {
		...
		public function getAdd() {
			...
			$data = [];
			$data['page_title'] = 'Tạo mới biên bảng kiểm kê';
			$data += ['mode' => 'new', 'stock_ids' => $user->stock_id];
			$this->cbView('stock_taking_form', $data);
        }
        ...
        public function getEdit($id)
        {
            ...
            $user = DB::table('cms_users')->where('id', CRUDBooster::myId())->first();
            $data = [];
            $data['page_title'] = 'Sửa biên bảng kiểm kê';
        ...
        public function getDetail($id)
        {
            ...
            $user = DB::table('cms_users')->where('id', CRUDBooster::myId())->first();
            $data = [];
            $data['page_title'] = 'Xem biên bảng kiểm kê';
	    ...
	    */
        ...
        public function getResumeOrder(){
            ...
            $stock = DB::table('gold_stocks')->where('id', $order->stock_id)->first();
			$details = DB::table('gold_stocks_taking_detail as D')
				...
				->leftJoin('gold_product_groups as PG', 'I.product_group_id', '=', 'PG.id')
				...
				->select('D.id',
					...
					'D.notes')
                ->get();
            return ['order'=>$order, 'stock'=>$stock, 'details'=>$details];
        }
        ...
        public function postUpdateOrder() {
            ...
            try {
				...
				// $items = DB::table('gold_items')->whereRaw('deleted_at is null')->where('stock_id', $order['stock_id'])->get();
				$items = DB::table('gold_items')->whereRaw('deleted_at is null and qty <> 0')->get();
				if($items && count($items)){
					...
					foreach ($items as $detail) {
						...
						// 	'qty' => 0, 
						// 	'notes' => 'Kiểm kê số [' . $order['order_no'] . ']', 
						// 	'updated_at' => date('Y-m-d H:i:s'), 
						// 	'updated_by' => CRUDBooster::myId()
						// ]);
					}
                    // Log::debug('$balances = ' . Json::encode($balances));
                    DB::table('gold_stocks_taking_balance')->insert($balances);
				}
        ...
        public function postAddNewItem() {
            ...
            try {
                ...
                if( $order['id'] && intval($order['id']) > 0) // update order
                {
					...
					$order_no = 'KK' . $order_date->format('ymd');
                    // get new order no
				...
				$detail['balance_qty'] = $item['balance_qty'];
				$detail['actual_qty'] = $item['actual_qty'];
                ...
                if( $detail['id'] && intval($detail['id']) > 0) // update order
                {
					...
					unset($detail['id']);
					// Log::debug('Update gold_stocks_taking_detail = ' . Json::encode($detail));
					DB::table('gold_stocks_taking_detail')->where('id', $detail_id)->update($detail);
                }else{
                    $detail['created_by'] = CRUDBooster::myId();
                    // Log::debug('Add gold_items = ' . Json::encode($item));
                    $detail_id = DB::table('gold_stocks_taking_detail')->insertGetId($detail);
                }
            }
            catch( \Exception $e){
                DB::rollback();
        ...
        public function postRemoveItem() {
            ...
            try {
				...
				DB::table('gold_stocks_taking_detail')->where('id', $detail_id)->delete();
            }
            catch( \Exception $e){
                DB::rollback();
                Log::debug('PostAdd error $e = ' . Json::encode($e));
		...
		public function postUpdateItemNotes() {
            ...
            try {
				...
				DB::table('gold_stocks_taking_detail')->where('id', $detail_id)->update(['notes'=>$para['notes']]);
            }
            catch( \Exception $e){
                DB::rollback();
                Log::debug('PostAdd error $e = ' . Json::encode($e));
		...
		public function getSearchStock(){
			...
			$stock = DB::table('gold_stocks')->whereRaw('deleted_at is null')->where('code', $stock_code)->first();
			// $items = DB::table('gold_items as I')
            //     ->leftJoin('gold_products as P', 'I.product_id', '=', 'P.id')
            //     ->leftJoin('gold_product_types as PT', 'P.product_type_id', '=', 'PT.id')
            //     ->whereRaw('I.deleted_at is null AND I.qty <> 0')
            ...
            //         'I.gem_weight',
            //         'I.gold_weight',
			// 		'I.qty AS balance_qty',
			// 		DB::raw('0 AS actual_qty'),
            //         'P.product_type_id',
            //         'PT.name as product_type_name')
            //     ->get();
            return ['stock'=>$stock];
        ...
        public function getPrintOrder($id) {
			...
			// if(DB::table('gold_stocks_taking')->whereRaw('status = 1')->where('id', $id)->first()){
			// 	$input = base_path().'/app/Reports/rpt_taking.jasper';
			// }else{
				$input = base_path().'/app/Reports/rpt_taking.jasper';
			// }
		...
		public function getPrintOrderXlsx($id){
			...
			$filename = 'KK-'.time();
			$parameter = ['id'=>$id];
			$input = base_path().'/app/Reports/rpt_taking_xlsx.jasper';
            $output = public_path() . '/output_reports/' . $filename;
            ...
            return Response::make($file, 200,
                array(
                    ...
                    'Content-Disposition' => 'filename="ds-hang-kiem-ke.xlsx"'
                )
            );
        }
	    ...
	    */
	    public function hook_after_delete($id) {
			//Your code here
			DB::table('gold_stocks_taking_balance')->where('order_id', $id)->delete();
            // $order = DB::table('gold_stocks_taking')->where('id', $id)->first();    
            // // Log::debug('$order = ' . Json::encode($order));
            // if($order && $order->status == 1){
			// 	$items = DB::table('gold_stocks_taking_detail')->whereRaw('deleted_at is null')->where('order_id', $order->id)->get();
			// 	if($items && count($items)){
			// 		foreach ($items as $detail) {
			// 			DB::table('gold_items')->where('id', $detail->item_id)->update(['qty'=>0, 'notes'=>null]);
			// 		}
			// 	}

			// 	$items = DB::table('gold_stocks_taking_balance')->where('order_id', $order->id)->get();
			// 	if($items && count($items)){
			// 		foreach ($items as $detail) {
			// 			DB::table('gold_items')->where('id', $detail->item_id)->update([
			// 				'qty' => $detail->qty, 
			...
			// 		}
			// 	}
			// 	DB::table('gold_stocks_taking_balance')->where('order_id', $order->id)->delete();
            // }
	    }
