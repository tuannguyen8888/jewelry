	...
	<div>
		<div class='panel panel-default'>
			<div class='panel-heading'>
				<strong><i class="fa fa-book"></i> Báo cáo tồn kho</strong>
			</div>
        ...
        function printReport(detail) {
            if(!$('#to_date').val()){
				...
				if(stock_ids) {
					...
					if(detail){
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalance')}}/" + to_date + "@" + $('#brand_id').val() + "@" + stock_ids,"print");
					}else{
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalanceAll')}}/" + to_date + "@" + $('#brand_id').val() + "@" + stock_ids,"print");
					}
				}else{
		...
		function exportReport(detail) {
            if(!$('#to_date').val()){
				...
				if(stock_ids) {
					...
					// console.log('to_date = ', to_date);
					if(detail){
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalanceXlsx')}}/" + to_date + "@" + $('#brand_id').val() + "@" + stock_ids,"print");
					}else{
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalanceAllXlsx')}}/" + to_date + "@" + $('#brand_id').val() + "@" + stock_ids,"print");
					}
				}else{
					alert("Bạn phải chọn ít nhất 1 kho!");
				}
