<?php namespace App\Http\Controllers;
	...
	class AdminGoldTransferBalanceController extends CBExtendController {
		...
		public function getAdd() {
			$data = [];
			$data['page_title'] = 'Tạo mới phiếu chuyển đổi công nợ';
			$data += ['mode' => 'new'];
			$this->cbView('transfer_balance_form', $data);
        }

        public function getEdit($id)
        {
            $data = [];
            $data['page_title'] = 'Sửa phiếu chuyển đổi công nợ';
        ...
        {
            $data = [];
            $data['page_title'] = 'Xem phiếu chuyển đổi công nợ';
	    ...
	    */
	    public function hook_query_index(&$query) {
	        //Your code here
			// $query->where('gold_transfer_balance.order_type', 0);
	    ...
	    */
        ...
        public function getResumeOrder(){
			...
			if($order->object_type == 0){
				...
				$object = DB::table('cms_users')->where('id', $order->object_id)->select('id', 'employee_code as code', 'name', 'q10', 'balance')->first();
			}
        ...
        public function postAddSave() {
            ...
            try {
                ...
                if($order['id'] && intval($order['id']) > 0) // update order
                {
					...
					$order_no = 'CĐ' . $order_date->format('ymd');
                    // get new order no
		...
		public function getSearchObject(){
			...
			if($object_type == 0){
				$object = DB::table('gold_customers')->whereRaw('deleted_at is null')->where('code', $object_code)->first();
			}else if($object_type == 1){
				$object = DB::table('gold_suppliers')->whereRaw('deleted_at is null')->where('code', $object_code)->first();
			}else if($object_type == 2){
				$object = DB::table('gold_investors')->whereRaw('deleted_at is null')->where('code', $object_code)->first();
			}else{
				$object = DB::table('cms_users')->whereRaw('status = 0')->where('employee_code', $object_code)->first();
			}
        ...
        public function getPrintOrder($id) {
            ...
            $input = base_path().'/app/Reports/rpt_transfer_balance.jasper';
	    ...
	    */
	    public function hook_after_delete($id) {
			//Your code here
            $order = DB::table('gold_transfer_balance')->where('id', $id)->first();    
            // Log::debug('$order = ' . Json::encode($order));
