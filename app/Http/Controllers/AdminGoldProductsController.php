<?php namespace App\Http\Controllers;
	...
	class AdminGoldProductsController extends CBExtendController {
	    ...
	    public function cbInit() {
			...
			$this->title_field = "product_code";
			$this->limit = "20";
			$this->orderby = "product_code,asc";
			...
			$this->col = [];
			$this->col[] = ["label"=>"Mã SP","name"=>"product_code"];
            $this->col[] = ["label"=>"Tên SP","name"=>"product_name"];
            $this->col[] = ["label"=>"Hình","name"=>"photo","image"=>true];
            ...
            $this->col[] = ["label"=>"HLV","name"=>"product_type_id","join"=>"gold_product_types,name"];
            $this->col[] = ["label"=>"Số lượng","name"=>"(select sum(qty) from gold_items where product_id = gold_products.id) as qty","callback_php"=>'number_format($row->qty);'];
            $this->col[] = ["label"=>"Q10","name"=>"(select sum(q10) from gold_items where product_id = gold_products.id AND qty > 0) as q10","callback_php"=>'number_format($row->q10, 4);'];
            $this->col[] = ["label"=>"Tình trạng","name"=>"(select case when sum(qty) <= 0 or sum(qty) is null then 0 else 1 end from gold_items where product_id = gold_products.id) as status","callback_php"=>'get_product_status($row->status);'];
			...
			$this->form = [];
			$this->form[] = ['label'=>'Mã sản phẩm','name'=>'product_code','type'=>'text','validation'=>'required|min:1|max:20','width'=>'col-sm-4'];
            $this->form[] = ['label'=>'Tên sản phẩm','name'=>'product_name','type'=>'text','validation'=>'required|min:1|max:200','width'=>'col-sm-10'];
            $this->form[] = ['label'=>'Hình','name'=>'photo','type'=>'upload','help'=>'Kích thước được đề xuất là 200x200 px','validation'=>'image|max:1000','resize_width'=>200,'resize_height'=>200,'width'=>'col-sm-4'];
            ...
            $this->form[] = ['label'=>'KL tổng','name'=>'total_weight','type'=>'text','validation'=>'required|numeric|min:0','width'=>'col-sm-4','onchange'=>'weight_onchange();'];
			$this->form[] = ['label'=>'KL đá','name'=>'gem_weight','type'=>'text','validation'=>'required|numeric|min:0','width'=>'col-sm-4','onchange'=>'weight_onchange();'];
            ...
            $this->form[] = ['label'=>'Công VIP','name'=>'whole_fee','type'=>'money','validation'=>'min:0','width'=>'col-sm-4'];
            if(CRUDBooster::myPrivilegeId() != 2)// Nhân viên bán hàng
            {
	        ...
	        */
            ...
            //$this->addaction[] = ['label'=>'In tem','url'=>CRUDBooster::mainpath('print-tem?id=[id]'),'icon'=>'fa fa-print','color'=>'info'];
	        ...
	        */
	        // $this->script_js = NULL;
            // $this->script_js = "function weight_onchange(){
            ...
            //     $('#gold_weight').val(total_weight - gem_weight);
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
            //Your code here
			$postdata['whole_fee'] = $postdata['whole_fee'] ? $postdata['whole_fee'] : 0;
	    }
...
//                    Log::debug('$rows = ',[Json::encode($rows)]);
                ...
                if(in_array('ma_hang', $data_import_column, true)
                    ...
                    && in_array('hlv', $data_import_column, true)
...
//                    Log::debug('File dung dinh dang');
                    ...
                    $data_import_column = ['Mã hàng', 'Tên hàng', 'ĐVT', 'LK Tổng', 'KL Đá', 'KL vàng', 'Công bán', 'Công VIP', 'Công vốn', 'HLV', 'Loại hàng', 'Nhóm hàng', 'Nhà sản xuất'];
...
//                    Log::debug('$value[$sk] = '.$value[$sk]);
                ...
                if(!$import_row['product_code']){
                    Cache::increment('success_'.$file_md5);
                    continue;
                }
                try{
                    $row_exists = DB::table($this->table)->where('product_code', $import_row['product_code'])->first();
                    if(!$row_exists) {
                        if($has_created_at) {
        ...
        public function getSearchProduct(){
            ...
            $para = Request::all();
            $product_code = $para['product_code'];
            $product = DB::table('gold_products as P')
                ...
                ->leftJoin('gold_product_categories as PC', 'P.product_category_id', '=', 'PC.id')
                ->leftJoin('gold_suppliers as S', 'P.supplier_id', '=', 'S.id')
                ->leftJoin('gold_product_units as U', 'P.product_unit_id', '=', 'U.id')
