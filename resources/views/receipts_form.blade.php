				...
				<form method='post' action='{{CRUDBooster::mainpath('add-save')}}' id="form">
					...
					<div class="col-sm-12">
                        <div class="row">
                            ...
                            <label class="control-label col-sm-2 text-right">T/g thu <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                        ...
                        <div class="row">
                            <label class="control-label col-sm-2">Hình thức thu</label>
                            <div class="col-sm-2">
			...
			<div class="box-footer" style="background: #F5F5F5">
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						@endif
						<a id="print_order" style="display: none;cursor: pointer;" onclick="printOrder()" class="btn btn-info"><i class="fa fa-print"></i> Phiếu thu</a>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="application/javascript">
        ...
        function searchObject() {
			...
			if(object_code && object_code.trim() != ''){
                $.ajax({
                    method: "GET",
                    url: '{{Route("AdminGoldReceiptsControllerGetSearchObject")}}',
                    data: {
        ...
        function showModalobject_id() {
            var url_object_id = "{{action('AdminGoldReceiptsController@getModalData')}}/gold_receipts/modal-data?table=gold_customers&columns=id,code,name&name_column=object_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã khách hàng,Tên khách hàng";  
            var object_type = $('#object_type').val() ? Number($('#object_type').val()) : 0;
            console.log('object_type = ', object_type);
            if(object_type == 1){
                url_object_id = "{{action('AdminGoldReceiptsController@getModalData')}}/gold_receipts/modal-data?table=gold_suppliers&columns=id,code,name&name_column=object_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã nhà cung cấp,Tên nhà cung cấp";    
            }else if(object_type == 2){
                url_object_id = "{{action('AdminGoldReceiptsController@getModalData')}}/gold_receipts/modal-data?table=gold_investors&columns=id,code,name&name_column=object_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã nhà đầu tư,Tên nhà đầu tư";    
        ...
        function printOrder() {
            if(order_id) {
                popupWindow("{{action('AdminGoldReceiptsController@getPrintOrder')}}/" + order_id,"print");
            }else{
