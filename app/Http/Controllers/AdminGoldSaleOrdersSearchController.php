<?php namespace App\Http\Controllers;
	...
	class AdminGoldSaleOrdersSearchController extends CBExtendController {
	    ...
	    public function cbInit() {
           ...
           // $this->col[] = ["label"=>"Số tiền","name"=>DB::raw('(gold_amount + fee - discount_amount - reduce) as amount'),"callback_php"=>'number_format($row->amount)'];
            ...
            //$this->col[] = ["label"=>"Nhân viên BH","name"=>"saler_id","join"=>"cms_users,name"];
            $this->col[] = ["label"=>"TL Tổng","name"=>"actual_weight","width"=>"100"];
            $this->col[] = ["label"=>"TL Đá","name"=>"id","join"=>"gold_sale_order_details,edit_weight"];
           ...
           // $this->search_form[] = ["label"=>"Loại đơn hàng", "name"=>"order_type","type"=>"select","width"=>"col-sm-2", 'dataenum'=>"0|<lable class='label label-warning'>Đang nhập</lable>;1|<lable class='label label-danger'>Hoàn tất</lable>"];
            if(CRUDBooster::myPrivilegeId() == 2) {
                ...
                $this->search_form[] = ["label" => "Phone", "name" => "customer_id", "type" => "select2", "width" => "col-sm-2", 'datatable' => 'gold_customers,name', 'datatable_where' => 'deleted_at is null', 'datatable_format' => "zalo_phone,' - ',IFNULL(phone,'')"];
            ...
            //$this->search_form[] = ["label"=>"TL Tổng", "name"=>"actual_weight","type"=>"select","width"=>"col-sm-2", 'datatable' => 'gold_sale_oders,name', 'datatable_where' => 'deleted_at is null'];
            //$this->search_form[] = ["label"=>"TL Đá", "name"=>"actual_weight","type"=>"select","width"=>"col-sm-2", 'datatable' => 'gold_sale_oders,name', 'datatable_where' => 'deleted_at is null'];
            $this->search_form[] = ["label"=>"TL Tổng", "name"=>"actual_weight","type"=>"select2","width"=>"col-sm-2", 'datatable'=>'gold_sale_orders,actual_weight', 'datatable_where'=>'deleted_at is null', 'datatable_format'=>"actual_weight"];
            // //$this->search_form[] = ["label"=>"TL Thuc te", "name"=>"edit_weight","type"=>"select2","width"=>"col-sm-2", 'datatable'=>'gold_sale_order_details,edit_weight', 'datatable_where'=>'deleted_at is null', 'datatable_format'=>"edit_weight"];
            ...
            //$this->search_form[] = ["label"=>"TL Tổng", "name"=>"actual_weight","type"=>"select2","width"=>"col-sm-2",  'datatable' => 'gold_sale_oders,actual_weight', 'datatable_where' => 'deleted_at is null', 'datatable_format' => "code,' - ',name,' - ',IFNULL(phone,'')"];
