<?php namespace App\Http\Controllers;
	...
	class AdminGoldStocksTransferController extends CBExtendController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Xuất tại kho","name"=>"from_stock_id","join"=>"gold_stocks,name"];
			$this->col[] = ["label"=>"Nhập tại kho","name"=>"to_stock_id","join"=>"gold_stocks,name"];
			...
			$this->form[] = ['label'=>'T/g chuyển','name'=>'order_date','type'=>'datetime','validation'=>'required|date_format:Y-m-d H:i:s','width'=>'col-sm-4'];
			$this->form[] = ['label'=>'Xuất tại kho','name'=>'from_stock_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'gold_stocks,name'];
			$this->form[] = ['label'=>'Nhập tại kho','name'=>'to_stock_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'gold_stocks,name'];
	        ...
	        */
            ...
            $this->addaction[] = ['label'=>'Phiếu chuyển','url'=>CRUDBooster::mainpath('print-order/[id]'),'icon'=>'fa fa-print','color'=>'info', 'showIf'=>"[status] != 0"];
		...
		public function getAdd() {
			$data = [];
			$data['page_title'] = 'Tạo mới phiếu chuyển';
			$data += ['mode' => 'new'];
			$this->cbView('stock_transfer_form', $data);
        }

        public function getEdit($id)
        {
            $data = [];
            $data['page_title'] = 'Sửa phiếu chuyển';
        ...
        {
            $data = [];
            $data['page_title'] = 'Xem phiếu chuyển';
	    ...
	    */
        ...
        public function getResumeOrder(){
			...
			$details = DB::table('gold_stocks_transfer_detail as D')
        ...
        public function postAddNewItem() {
            ...
            try {
                ...
                if( $order['id'] && intval($order['id']) > 0) // update order
                {
					...
					$order_no = 'CK' . $order_date->format('ymd');
                    // get new order no
                ...
                if($detail_id > 0) // update order
                {
					$detail['updated_at'] = date('Y-m-d H:i:s');
                	$detail['updated_by'] = CRUDBooster::myId();
					// Log::debug('Update gold_stocks_transfer_detail = ' . Json::encode($detail));
					DB::table('gold_stocks_transfer_detail')->where('id', $detail_id)->update($detail);
                }else{
                    $detail['created_by'] = CRUDBooster::myId();
                    // Log::debug('Add gold_items = ' . Json::encode($item));
                    $detail_id = DB::table('gold_stocks_transfer_detail')->insertGetId($detail);
                }
            }
            catch( \Exception $e){
                DB::rollback();
        ...
        public function postRemoveItem() {
            ...
            try {
				...
				DB::table('gold_stocks_transfer_detail')->where('id', $detail_id)->delete();
            }
            catch( \Exception $e){
                DB::rollback();
                Log::debug('PostAdd error $e = ' . Json::encode($e));
		...
		public function postUpdateDetail() {
            ...
            try {
                ...
                $detail['updated_by'] = CRUDBooster::myId();
                unset($detail['id']);
				DB::table('gold_stocks_transfer_detail')->where('id', $id)->update($detail);
            }
            catch( \Exception $e){
                DB::rollback();
                Log::debug('PostAdd error $e = ' . Json::encode($e));
	    ...
	    */
	    public function hook_after_delete($id) {
			//Your code here
            $order = DB::table('gold_stocks_transfer')->where('id', $id)->first();    
            // Log::debug('$order = ' . Json::encode($order));
