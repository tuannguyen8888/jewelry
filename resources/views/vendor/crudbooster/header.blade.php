...
<header class="main-header">
    ...
    <nav class="navbar navbar-static-top" role="navigation">
        ...
        {{--<a href="#" class="logo">--}}
            {{--<span>{{ CRUDBooster::myBrandName() }}</span>--}}
        {{--</a>--}}
        <!-- Navbar Right Menu -->
        <div class="navbar-custom-menu">
            <ul class="nav navbar-nav">
                <li class="dropdown branchs-menu tasks-menu">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" title='Chọn của hàng' aria-expanded="false">
                        <span>{{ CRUDBooster::myBrandName() }}</span> <i id='icon_branchs' class="fa fa-building"></i>
                    </a>
                    <ul id='list_branchs' class="dropdown-menu">
                        <li class="header text-bold">Chọn của hàng</li>
                        <li>
                            <!-- inner menu: contains the actual data -->
                            <div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 200px;">
                                <ul class="menu" style="overflow: hidden; width: 100%; height: 200px;">
                                    @foreach(CRUDBooster::myBrands() as $brand)
                                    <li >
                                        @if($brand->id == CRUDBooster::myBrand())
                                            <em class="text-blue" style="padding-left: 20px;"> <i id='icon_branchs' class="fa fa-check-circle-o"></i> {{$brand->name}}</em>
                                        @else
                                            <a href="javascript:void(0);" onclick="switchToBrand({{$brand->id}});">
                                                <em style="padding-left: 20px;">{{$brand->name}}</em>
                                            </a>
                                        @endif
                                    </li>
                                    @endforeach
                        ...
                        @push('bottom')
                            <script type="application/javascript">
                                function switchToBrand(brand_id) {
                                    $.ajax({
                                        url: '{{Route("AdminCmsUsersControllerPostSwitchToBrand")}}',
                                        type: 'POST',
                                        dataType: 'json',
                                        data: {
                                            brand_id: brand_id,
                                        ...
                                        success: function (data) {
                                            console.log('switchToBrand result = ',data);
                                            if(data && data.result){
                                        ...
                                        error: function (e) {
                                            console.log(e);
                                            swal("Xảy ra lỗi khi chuyển cửa hàng", "Vui lòng thử lại, hoặc liên hệ kỹ thuật.", "error");
                                        }
                                    });
                                }
                            </script>
