...
class AdminTransferMoneyController extends CBExtendController {
...
//        $this->col[] = ["label"=>"% phí DV","name"=>"fee","callback_php"=>'number_format($row->fee, 2)'];
        ...
        $this->form[] = ['label'=>'T/g thực hiện','name'=>'order_date','type'=>'datetime','validation'=>'required|date_format:Y-m-d H:i:s','width'=>'col-sm-4'];
        ...
        */
        ...
        $this->addaction[] = ['label'=>'Phiếu chuyển tiền','url'=>CRUDBooster::mainpath('print-order/[id]'),'icon'=>'fa fa-print','color'=>'info', 'showIf'=>"[status] != 0"];
    ...
    public function getAdd() {
        $data = [];
        $data['page_title'] = 'Tạo mới phiếu chuyển tiền';
        $data += ['mode' => 'new'];
        $this->cbView('transfer_money_form', $data);
    }

    public function getEdit($id)
    {
        $data = [];
        $data['page_title'] = 'Sửa phiếu chuyển tiền';
    ...
    {
        $data = [];
        $data['page_title'] = 'Xem phiếu chuyển tiền';
    ...
    */
    ...
    public function postAddSave() {
        ...
        try {
            ...
            //$amount = round($order['amount']  + $order['fee']);
            ...
            $order['method'] = 1;
            $order['order_type'] = 3; // chuyển tiền
            ...
            if($order['id'] && intval($order['id']) > 0) // update order
            {
                ...
                $order_no = 'CT' . $order_date->format('ymd');
                // get new order no
