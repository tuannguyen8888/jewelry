				...
				<form method='post' action='{{CRUDBooster::mainpath('add-save')}}' id="form">
					...
					<div class="col-sm-12">
                        <div class="row">
                            ...
                            <label class="control-label col-sm-2 text-right">T/g chuyển <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                        ...
                        <div class="row">
							...
							<div class="col-sm-10">
								<div class="input-group">
									...
									<input type="text" name="object_code" id="object_code" onchange="searchObject();" class="form-control" required style="width: 10%">
									<input type="text" name="object_name" id="object_name" class="form-control" placeholder="Tên đối tượng" style="width: 65%" disabled>
                                    <input type="text" name="object_q10" id="object_q10" class="form-control money" placeholder="Q10" style="width: 10%" disabled>
                                    <input type="text" name="object_balance" id="object_balance" class="form-control money" placeholder="Công nợ" style="width: 15%" disabled>
									<input type="hidden" name="object_id" id="object_id">
								</div>
							</div>
						</div>
                        <div class="row">
                            <label class="control-label col-sm-2">Q10 <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                                <input type="text" name="q10" id="q10" class="form-control money" onchange="priceChange();">
                            </div>
                            <label class="control-label col-sm-2 text-right">Đơn giá <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                                <input type="text" name="price" id="price" class="form-control money" onchange="priceChange();">
                            </div>
                            <label class="control-label col-sm-2 text-right">Thành tiền</label>
                            <div class="col-sm-2">
                                <input type="text" name="amount" id="amount" class="form-control money" disabled>
                            </div>
                        </div>
                        <div class="row">
			...
			<div class="box-footer" style="background: #F5F5F5">
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						@endif
						<a id="print_order" style="display: none;cursor: pointer;" onclick="printOrder()" class="btn btn-info"><i class="fa fa-print"></i> Phiếu chuyển</a>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="application/javascript">
                ...
                success: function (data) {
                    if (data){
                        if(data.object){
                            ...
                            AutoNumeric.getAutoNumericElement('#object_q10').set(data.object.q10);
                            AutoNumeric.getAutoNumericElement('#object_balance').set(data.object.balance);
                        }
						if(data.order){
                            ...
                            AutoNumeric.getAutoNumericElement('#q10').set(data.order.q10);
                            AutoNumeric.getAutoNumericElement('#price').set(data.order.price);
        ...
        function searchObject() {
			...
			if(object_code && object_code.trim() != ''){
                $.ajax({
                    method: "GET",
                    url: '{{Route("AdminGoldTransferBalanceControllerGetSearchObject")}}',
                    data: {
        ...
        function objectTypeChange() {
            ...
            $('#object_name').val(null);
            AutoNumeric.getAutoNumericElement('#object_q10').set(0);
            AutoNumeric.getAutoNumericElement('#object_balance').set(0);
            $('#object_code').focus();
        }

        function priceChange() {
            let q10 = $('#q10').val() ? AutoNumeric.getAutoNumericElement('#q10').getNumber() : 0;
            let price = $('#price').val() ? AutoNumeric.getAutoNumericElement('#price').getNumber() : 0;
            AutoNumeric.getAutoNumericElement('#amount').set(Math.round(q10 * price));
        }

        function showModalobject_id() {
            var url_object_id = "{{action('AdminGoldTransferBalanceController@getModalData')}}/gold_transfer_balance/modal-data?table=gold_customers&columns=id,code,name&name_column=object_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã khách hàng,Tên khách hàng";  
            var object_type = $('#object_type').val() ? Number($('#object_type').val()) : 0;
            console.log('object_type = ', object_type);
            if(object_type == 1){
                url_object_id = "{{action('AdminGoldTransferBalanceController@getModalData')}}/gold_transfer_balance/modal-data?table=gold_suppliers&columns=id,code,name&name_column=object_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã nhà cung cấp,Tên nhà cung cấp";    
            }else if(object_type == 2){
                url_object_id = "{{action('AdminGoldTransferBalanceController@getModalData')}}/gold_transfer_balance/modal-data?table=gold_investors&columns=id,code,name&name_column=object_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã nhà đầu tư,Tên nhà đầu tư";    
            }else if(object_type == 3){
                if(Number('{{CRUDBooster::myPrivilegeId()}}') == 1){
                    url_object_id = "{{action('AdminGoldTransferBalanceController@getModalData')}}/gold_transfer_balance/modal-data?table=cms_users&columns=id,employee_code,name&name_column=object_id&where=status+=+0&select_to=employee_code:code&columns_name_alias=Mã nhân viên,Tên nhân viên";
                }else{
                    url_object_id = "{{action('AdminGoldTransferBalanceController@getModalData')}}/gold_transfer_balance/modal-data?table=cms_users&columns=id,employee_code,name&name_column=object_id&where=status+=+0+and+id+<>+1&select_to=employee_code:code&columns_name_alias=Mã nhân viên,Tên nhân viên";
                }
            }
        ...
        function getOrderHeader(finish) {
            return {
                ...
                notes: $('#notes').val(),
                q10: $('#q10').val() ? AutoNumeric.getAutoNumericElement(`#q10`).getNumber() : 0,
                price: $('#price').val() ? AutoNumeric.getAutoNumericElement(`#price`).getNumber() : 0,
        ...
        function printOrder() {
            if(order_id) {
                popupWindow("{{action('AdminGoldTransferBalanceController@getPrintOrder')}}/" + order_id, "print");
            }else{
                alert("Bạn không thể in phiếu thu nếu chưa lưu đơn hàng!");
            }
        }
	</script>
@endpush