				...
				<form method='post' action='{{CRUDBooster::mainpath('add-save')}}' id="form">
					...
					<div class="col-sm-12">
                        ...
                        <div class="row">
                            ...
                            <label class="control-label col-sm-2 text-right">T/g xuất <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                                <div class="input-group" >
		...
		<div class="modal-dialog modal-md " role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title"><i class="fa fa-search"></i> Delete Barcode | Hàng đang xuất</h4>
				</div>
	...
	<script type="application/javascript">
                ...
                if(moment().diff(lastTimeScanBarCode, 's') >= 1) // productFinding &&
                {
                    ...
                    if(bar_code){
                        ...
                        if(!added) {
                            ...
                            $.ajax({
                                method: "GET",
                                url: '{{Route("AdminGoldStocksIssuesControllerGetSearchItem")}}',
                                data: {
                                    bar_code: bar_code,
                                    _token: '{{ csrf_token() }}'
                                },
        ...
        function addNewOrderDetail(dataRow) {
            let html = `<tr id="order_detail_${dataRow.id}">
                    ...
                    <th class="age no-padding"><input id="age_${dataRow.id}" type="text" class="form-control money" value="${dataRow.age}" onchange="age_change(${dataRow.id})"></th>
                    <th class="text-right" id="q10_${dataRow.id}">${dataRow.q10.toLocaleString('en-US')}</th>
                    <th class="fee no-padding"><input id="fee_${dataRow.id}" type="text" class="form-control money" value="${dataRow.fee}" onchange="fee_change(${dataRow.id})"></th>
                    <th class="no-padding"><input id="notes_${dataRow.id}" type="text" class="form-control" value="${dataRow.notes ? dataRow.notes : ''}" onchange="notes_change(${dataRow.id})"></th>
                </tr>`;
        ...
        function notes_change(id) {
            $.ajax({
                method: "POST",
                url: '{{CRUDBooster::mainpath('update-detail')}}',
                data: {
                    detail: {id: id, notes: $('#notes_'+id).val()},
                    _token: '{{ csrf_token() }}'
                },
                dataType: "json",
                async: true,
        ...
        function age_change(id) {
            order_details.forEach(function (detail, index) {
                if(detail.id == id){
                    detail.age = $('#age_'+id).val() ? Number($('#age_'+id).val().replace(/,/g, '')) : 0;
                    detail.q10 = Math.round((detail.gold_weight * detail.age / 100) * 10000) / 10000;
                    $(`#q10_${id}`).html(detail.q10.toLocaleString('en-US'));
                    calcTotalOfOrderDetails();
                    $.ajax({
                        method: "POST",
                        url: '{{CRUDBooster::mainpath('update-detail')}}',
                        data: {
                            detail: {id: id, age: detail.age, q10: detail.q10},
                            _token: '{{ csrf_token() }}'
                        },
                        dataType: "json",
                        async: true,
        ...
        function fee_change(id) {
            order_details.forEach(function (detail, index) {
                if(detail.id == id){
                    detail.fee = $('#fee_'+id).val() ? Number($('#fee_'+id).val().replace(/,/g, '')) : 0;
                    calcTotalOfOrderDetails();
                    $.ajax({
                        method: "POST",
                        url: '{{CRUDBooster::mainpath('update-detail')}}',
                        data: {
                            detail: {id: id, fee: detail.fee},
                            _token: '{{ csrf_token() }}'
                        },
                        dataType: "json",
                        async: true,
        ...
        function validate() {
            ...
            //     valid = false;
            //     $('#supplier_code').addClass('invalid');
            // }
            if(!$('#order_date').val()){
                valid = false;
            ...
            if(!valid) {
                ...
                valid = false;
                $(`#bar_code`).focus();
                swal("Thông báo", "Không có hàng hóa xuất kho, vui lòng kiểm tra lại.", "warning");
            }
			return valid;
        }
        ...
        function printOrder() {
            if(order_id) {
                popupWindow("{{action('AdminGoldStocksIssuesController@getPrintOrder')}}/" + order_id,"print");
            }else{
