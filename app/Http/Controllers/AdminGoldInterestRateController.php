<?php namespace App\Http\Controllers;
	...
	class AdminGoldInterestRateController extends CBExtendController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Thời hạn (ngày)","name"=>"due_date","callback_php"=>'number_format($row->due_date)'];
			$this->col[] = ["label"=>"T/h tối thiểu (ngày)","name"=>"min_days","callback_php"=>'number_format($row->min_days)'];
			$this->col[] = ["label"=>"Lãi suất (%)","name"=>"rate","callback_php"=>'number_format($row->rate, 2)'];
			...
			$this->form[] = ['label'=>'Thời hạn (ngày)','name'=>'due_date','type'=>'money','validation'=>'required|min:0','width'=>'col-sm-4'];
			$this->form[] = ['label'=>'T/h tối thiểu (ngày)','name'=>'min_days','type'=>'money','validation'=>'required|min:0','width'=>'col-sm-4'];
			$this->form[] = ['label'=>'Lãi suất (%)','name'=>'rate','type'=>'text','validation'=>'required|numeric|min:0','width'=>'col-sm-4'];
	    ...
	    */
		...
		public function getRate(){
            ...
            $rate = DB::table('gold_interest_rate')
				...
				->select('id',
					...
					'rate')
				->first();
			Log::debug('$rate = ' . Json::encode($rate));		
            return ['rate'=>$rate];
		}
	}