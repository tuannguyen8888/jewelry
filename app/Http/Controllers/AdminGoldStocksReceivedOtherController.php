<?php namespace App\Http\Controllers;
	...
	class AdminGoldStocksReceivedOtherController extends CBExtendController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Nhân viên","name"=>"supplier_id","join"=>"cms_users,name"];
	        ...
	        */
            ...
            $this->addaction[] = ['label'=>'Phiếu NK','url'=>CRUDBooster::mainpath('print-received/[id]'),'icon'=>'fa fa-file-text-o','color'=>'info', 'showIf'=>"[status] != 0"];
		...
		public function getAdd() {
			$data = [];
			$data['page_title'] = 'Tạo mới phiếu nhập dẻ';
			$data += ['mode' => 'new'];
			$this->cbView('stock_received_other_form', $data);
        }

        public function getEdit($id)
        {
            $data = [];
            $data['page_title'] = 'Sửa phiếu nhập dẻ';
        ...
        {
            $data = [];
            $data['page_title'] = 'Xem phiếu nhập dẻ';
	    ...
	    */
        ...
        public function getResumeReceived(){
            ...
            $items = DB::table('gold_items as I')
                ...
                ->get();
            return ['received'=>$received, 'total'=>$total, 'items'=>$items];
        }
        ...
        public function postUpdateReceivedHeader() {
            ...
            try {
                ...
                $user = DB::table('cms_users')->where('id', $update_received['supplier_id'])->first();
                if($user){
                    $total = $this->totalReceived($update_received['id']);
                    DB::table('cms_users')->where('id', $user->id)->update([
                        'q10' => $user->q10 - ($total->q10 ? $total->q10 : 0)
                    ]);
                }
        ...
        public function getPrintReceived($id) {
            ...
            $input = base_path().'/app/Reports/rpt_received_other.jasper';
        ...
        public function getPrintReceivedDetail($id) {
            ...
            $input = base_path().'/app/Reports/rpt_received_other_detail.jasper';
            $output = public_path().'/output_reports/'.$filename;
            $jasper->process($input, $output, array('pdf'), $parameter, $database)->execute();
	    ...
	    */
	    public function hook_after_delete($id) {
            ...
            if($received && $received->status == 1){
                $user = DB::table('cms_users')->where('id', $received->supplier_id)->first();    
                // Log::debug('$supplier = ' . Json::encode($supplier));
                if($user){
                    $total = $this->totalReceived($id);
                    DB::table('cms_users')->where('id', $user->id)->update([
                        'q10' => $user->q10 + $total->q10
                    ]);
                }
            }
