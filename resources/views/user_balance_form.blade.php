	...
	<div>
		<div class='panel panel-default'>
			<div class='panel-heading'>
				<strong><i class="fa fa-bar-chart-o"></i>Công Nợ Nhân Viên</strong>
			</div>
        ...
        function printReport(print) {
            if(!$('#from_date').val()){
				...
				if(ids) {
					...
					if(print){
						if($('#rpt_type').val() == 0){
							popupWindow("{{action('AdminCmsUsersController@getPrintBalance')}}/" + to_date + "@" + $('#brand_id').val() + "@" + ids,"print");
						}else{
							popupWindow("{{action('AdminCmsUsersController@getPrintBalanceDetail')}}/" + from_date + "@" + to_date + "@" + $('#brand_id').val() + "@" + ids,"print");
						}
					}else{
						if($('#rpt_type').val() == 0){
							popupWindow("{{action('AdminCmsUsersController@getPrintBalanceXlsx')}}/" + to_date + "@" + $('#brand_id').val() + "@" +ids,"export");
						}else{
							popupWindow("{{action('AdminCmsUsersController@getPrintBalanceDetailXlsx')}}/" + from_date + "@" + to_date + "@" + $('#brand_id').val() + "@" +ids,"export");
						}
					}
				}else{
