        ...
        function printReport(detail) {
            if(!$('#to_date').val()){
				...
				if(stock_ids) {
					...
					if(detail){
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalance')}}/" + to_date + "@" + stock_ids,"print");
					}else{
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalanceAll')}}/" + to_date + "@" + stock_ids,"print");
					}
				}else{
		...
		function exportReport(detail) {
            if(!$('#to_date').val()){
				...
				if(stock_ids) {
					...
					// console.log('to_date = ', to_date);
					if(detail){
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalanceXlsx')}}/" + to_date + "@" + stock_ids,"print");
					}else{
						popupWindow("{{action('AdminGoldStocksController@getPrintStockBalanceAllXlsx')}}/" + to_date + "@" + stock_ids,"print");
					}
				}else{
					alert("Bạn phải chọn ít nhất 1 kho!");
				}
