<?php namespace App\Http\Controllers;
	...
	class AdminGoldDashboardController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
			$this->col[] = ["label"=>"Số sổ","name"=>"counter_no"];
            ...
            $this->col[] = ["label"=>"Tiền ứng","name"=>"advance_amount","callback_php"=>'number_format($row->advance_amount)'];
			$this->col[] = ["label"=>"Tiền bán (TM)","name"=>'sales_amount',"callback_php"=>'number_format($row->sales_amount)'];
			$this->col[] = ["label"=>"Tiền bán (CK)","name"=>'bank_amount',"callback_php"=>'number_format($row->bank_amount)'];
	    ...
	    */
	    public function hook_query_index(&$query) {
            ...
            {
				$query->where('gold_counters.saler_id', CRUDBooster::myId());
            }
            $from_date = date('Y-m-d');
			$query->whereRaw("(gold_counters.opened_at >= '" . $from_date . "' or gold_counters.closed_at is null or gold_counters.closed_at > '" . $from_date . "')");
			// $query->where('opened_at', '<=', $from_date . ' 23:59:59');
