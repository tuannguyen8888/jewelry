...
use Psy\Util\Json;
	...
	class CBExtendSearchController extends \crocodicstudio\crudbooster\controllers\CBController {
        ...
        public function getIndex() {
            ...
            if(Request::get('order_date_from_date') && Request::get('order_date_from_date')) {
                $result->where(function($w) use ($columns_table, $request) {;
                    foreach($columns_table as $col) {
                        if($col['field_with'] == 'gold_price.apply_time'){
                            $w->where($col['field_with'],">=", Request::get('order_date_from_date'));
                            $w->where($col['field_with'],"<=", Request::get('order_date_to_date') . ' 23:59:59');
                        }
        ...
        public function getDelete($id) {
            ...
            // edit by ThietNH
            $childs = DB::table('cms_foreign_key_mapping')->where('parent_table', $this->table)->get();
            if($childs){
                foreach ($childs as $detail) {
                    if($detail->cond){
                        if(DB::table($detail->child_table)->whereRaw($detail->cond)->where($detail->child_field, $id)->first()){
                            CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error',['error'=>CRUDBooster::getCurrentModule()->name]),'error');
                            continue;
                        }
                    }else{
                        if(DB::table($detail->child_table)->where($detail->child_field, $id)->first()){
                            CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error',['error'=>CRUDBooster::getCurrentModule()->name]),'error');
                            continue;
                        }
                    }
            ...
            if($this->table == "gold_purchase_orders" || $this->table == "gold_sale_orders" || $this->table == "gold_pawn_orders" || $this->table == "gold_pawn_order_interested" || $this->table == "gold_vouchers"){
                if(DB::table('gold_counters')->whereRaw('closed_at is not null')->where('id', $row->counter_id)->first()){
                    CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error_counter',['error'=>CRUDBooster::getCurrentModule()->name]),'error');
                }else if($this->table == "gold_pawn_orders"){
                    if(DB::table($this->table)->whereRaw('status = 2')->where($this->primary_key, $id)->first()){
                        CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error_pawn'),'error');
                    }else if(DB::table('gold_pawn_order_interested')->whereRaw('deleted_at is null')->where('order_id', $id)->first()){
                        CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error_pawn_interested'),'error');
                    }
                }else if($this->table == "gold_pawn_order_interested"){
                    $interested = DB::table($this->table)->where($this->primary_key, $id)->first();
                    if($interested){
                        if(DB::table('gold_pawn_order_interested')->whereRaw('deleted_at is null')
                            ->where('order_id', $interested->order_id)->where('id', '>', $interested->id)->first()){
                            CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error_pawn_interested'),'error');
                        }    
                    }
                }
            }else if($this->table == "gold_stocks_received"){
                $received = DB::table($this->table)->where($this->primary_key, $id)->first();
                if($received && $received->status == 1){
                    if(DB::table('gold_items')->whereRaw("(qty = 0 OR stock_id <> ".$received->stock_id.")")->where('received_id', $id)->first()){
                        CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error_item',['error'=>CRUDBooster::getCurrentModule()->name]),'error');
                    }
                }
            }else if($this->table == "gold_stocks_transfer"){
                $transfer = DB::table($this->table)->where($this->primary_key, $id)->first();
                if($transfer && $transfer->status == 1){
                    if(DB::table('gold_items')->whereRaw('(qty = 0 OR stock_id <> '.$transfer->to_stock_id.') AND id IN(SELECT item_id FROM gold_stocks_transfer_detail WHERE deleted_at is null AND order_id = '.$id.')')->first()){
                        CRUDBooster::redirect($url,trans('crudbooster.alert_delete_error_item',['error'=>CRUDBooster::getCurrentModule()->name]),'error');
                    }
                }
            }
