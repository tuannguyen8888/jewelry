			...
			<div class="panel-body" id="parent-form-area">
				<form method='post' action='{{CRUDBooster::mainpath('add-save')}}' id="form">
					...
					<div class="col-sm-12">
                        ...
                        <div class="row">
                            <label class="control-label col-sm-2">Loại thẻ <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                                <select id="card_type" class="form-control" onchange="card_type_change();"></select>
                            </div>
                            <label class="control-label col-sm-2 text-right">Thời gian rút <span class="text-danger" title="Không được bỏ trống trường này.">*</span></label>
                            <div class="col-sm-2">
                                <div class="input-group" >
                        ...
                        <div class="row">
                            ...
                            <label class="control-label col-sm-2 text-right">Tiền trả cho khách</label>
                            <div class="col-sm-2">
...
@push('bottom')
	...
	<script type="application/javascript">
                    ...
                    if (data && data.list && data.list.length > 0) {
						...
						data.list.forEach(function (detail, i) {
                            ...
                            if(i == 0){
                                ...
                                AutoNumeric.getAutoNumericElement(`#fee`).set(detail.fee);
                            }
                        });
                ...
                success: function (data) {
                    if (data){
						...
						if(data.order){
                            ...
                            AutoNumeric.getAutoNumericElement('#bank_amount').set(Math.round(data.order.amount * data.order.bank_fee / 100));
                            AutoNumeric.getAutoNumericElement('#fee').set(data.order.fee);
                            AutoNumeric.getAutoNumericElement('#fee_amount').set(Math.round(data.order.amount * data.order.fee / 100));
                            AutoNumeric.getAutoNumericElement('#return_amount').set(Math.round(data.order.amount * (1 - (data.order.bank_fee + data.order.fee) / 100)));
        ...
        function showModalcustomer_id() {
            var url_customer_id = "{{action('AdminGoldWithdrawalController@getModalData')}}/gold_purchase_orders/modal-data?table=gold_customers&columns=id,code,name,address,phone&name_column=customer_id&where=deleted_at+is+null&select_to=code:code&columns_name_alias=Mã khách hàng,Tên khách hàng,Địa chỉ,Số điện thoại";
        ...
        function amount_change() {
            ...
            let fee = $('#fee').val() ? AutoNumeric.getAutoNumericElement(`#fee`).getNumber() : 0;

            AutoNumeric.getAutoNumericElement(`#bank_amount`).set(Math.round(amount * bank_fee / 100));
            AutoNumeric.getAutoNumericElement(`#fee_amount`).set(Math.round(amount * fee / 100));
            AutoNumeric.getAutoNumericElement(`#return_amount`).set(Math.round(amount * (1 - (bank_fee + fee) / 100)));
        }
        ...
        function getOrderHeader(finish) {
            return {
                ...
                card_type_id: $('#card_type').val() ? Number($('#card_type').val()) : null,
        ...
        function printInvoice() {
            if(order_id) {
                popupWindow("{{action('AdminGoldWithdrawalController@getPrintOrder')}}/" + order_id,"print");
            }else{
